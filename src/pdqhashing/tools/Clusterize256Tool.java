 ...
 * produce cluster sizes on output. (Those need to be computed as an afterpass.)
 * It necessary for operating on millions of hashes.
 *
 *
 ...
 * listed within that cluster. This is transitive: if h1 is near h2, h2 is near
 * h3, and h3 is near h4, then all four are clustered together even if h1 is not
 * near h4.
 *
...
public class Clusterize256Tool {
	...
	public static void main(String[] args) {
		...
		// Clusterize
		if (snowball) {
			snowballClusterize(vectorOfPairs, mih, separateClusters, traceCount, doBruteForceQuery, distanceThreshold);
		} else {
			radiallyClusterize(vectorOfPairs, mih, separateClusters, traceCount, doBruteForceQuery, distanceThreshold);
		}
	}

	// ----------------------------------------------------------------
	private static void snowballClusterize(Vector<Hash256AndMetadata<String>> vectorOfPairs, MIH256<String> mih,
			boolean separateClusters, int traceCount, boolean doBruteForceQuery, int distanceThreshold) {
		...
		for (Map.Entry<String, Set<String>> entry : equivalenceClasses.entrySet()) {
			...
			for (String metadata : equivalenceClass) {
				System.out.printf("clidx=%d,clusz=%d,hash=%s,%s\n", clusterIndex, clusterSize,
						metadataToHashes.get(metadata).toString(), metadata);
			}
		}
	}

	// ----------------------------------------------------------------
	private static void radiallyClusterize(Vector<Hash256AndMetadata<String>> vectorOfPairs, MIH256<String> mih,
			boolean separateClusters, int traceCount, boolean doBruteForceQuery, int distanceThreshold) {
			...
			if (clusterSize > 0) {
				...
				for (Hash256AndMetadata<String> hayfiberPair : matches) {
					...
					System.out.printf("clidx=%d,clusz=%d,hash1=%s,hash2=%s,d=%d,%s\n", clusterIndex, clusterSize,
							needleHash.toString(), hayfiberHash.toString(), d, hayfiberMetadata);
				}
			}
