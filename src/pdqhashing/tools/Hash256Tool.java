...
/**
 * This is an ops tool for doing various things to 256-bit hashes with
	...
	static void doDeltas(String verb, String[] args) {
		...
		for (int i = 0; i < n; i++) {
			if (i == 0) {
				...
				System.out.printf("%s %d\n", hashes.get(i).toString(),
						hashes.get(i).hammingDistance(hashes.get(i - 1)));
			}
		}
	}
			...
			for (int i = 0; i < m; i++) {
				for (int j = 0; j < n; j++) {
					System.out.printf("ci=%s,cj=%s,i=%d,j=%d,d=%d\n", hashes1.get(i).toString(),
							hashes2.get(j).toString(), i, j, hashes1.get(i).hammingDistance(hashes2.get(j)));
				}
			}
		} else {
			for (int i = 0; i < m; i++) {
