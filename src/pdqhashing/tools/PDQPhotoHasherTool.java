...
public class PDQPhotoHasherTool {
	...
	public static void main(String[] args) {
		...
		Context context = new Context(0, // numPDQHash
				null, // pdqHashPrev
				false); // hadError
		...
		if (filesOnStdin) {
			...
			try {
				while ((filename = reader.readLine()) != null) { // In Java, line-terminators already stripped for us
					lno++;
					context.numPDQHash++;
					processFile(pdqHasher, filename, doPDQHash, doPDQDih, doPDQDihAcross, doDetailedOutput, doTimings,
							keepGoingAfterErrors, context);
					System.out.flush();
				}
			} catch (IOException e) {
	...
	// ----------------------------------------------------------------
	static void processFile(PDQHasher pdqHasher, String filename, boolean doPDQHash, boolean doPDQDih,
			boolean doPDQDihAcross, boolean doDetailedOutput, boolean doTimings, boolean keepGoingAfterErrors,
			Context context) {
		Hash256 hash = null;
		...
		if (doPDQHash) {
			...
			delta = (context.numPDQHash == 1) ? 0 : hash.hammingDistance(context.pdqHashPrev);
			...
			if (!doDetailedOutput) {
				if (doPDQDihAcross) {
					System.out.printf("%s,%s,%s,%s,%s,%s,%s,%s,%d,%s\n", dihedralBag.hash.toString(),
							dihedralBag.hashRotate90.toString(), dihedralBag.hashRotate180.toString(),
							dihedralBag.hashRotate270.toString(), dihedralBag.hashFlipX.toString(),
							dihedralBag.hashFlipY.toString(), dihedralBag.hashFlipPlus1.toString(),
							dihedralBag.hashFlipMinus1.toString(), dihedralBag.quality, filename);
				} else {
