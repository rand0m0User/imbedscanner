	...
	public static void main(String[] args) {
		...
		if (doInvert) {
			...
			for (Hash256AndMetadata<String> needlePair : needles) {
				...
				try {
					matchPair = doBruteForceQuery ? mih.bruteForceQueryAny(needlePair.hash, distanceThreshold)
				...
				if (matches.isEmpty()) {
					...
					for (Hash256AndMetadata<String> matchPair : matches) {
						System.out.printf("d=%d,needle=%s,%s,match=%s,%s\n",
								matchPair.hash.hammingDistance(needlePair.hash), needlePair.hash.toString(),
								needlePair.metadata, matchPair.hash.toString(), matchPair.metadata);
					}
				}
			}
			...
			for (Hash256AndMetadata<String> needlePair : needles) {
				...
				for (Hash256AndMetadata<String> matchPair : matches) {
					System.out.printf("d=%d,match=%s,%s\n", matchPair.hash.hammingDistance(needlePair.hash),
							matchPair.hash.toString(), matchPair.metadata);
				}
			}
