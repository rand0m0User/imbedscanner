...
public class PDQHasher {
	...
	// ----------------------------------------------------------------
	public HashAndQuality fromFile(String filename, HashingMetadata hashingMetadata) throws IOException {
		long t1, t2;
	...
	// ----------------------------------------------------------------
	// The buffers could be allocated within this method rather than being passed
	// in.
	...
	public HashAndQuality fromBufferedImage(BufferedImage img, float[] buffer1, // image numRows x numCols as row-major
																				// array
			float[] buffer2, // image numRows x numCols as row-major array
			float[][] buffer64x64, float[][] buffer16x64, float[][] buffer16x16) {
		int numRows = img.getHeight();
		int numCols = img.getWidth();

		fillFloatLumaFromBufferImage(img, buffer1);

		return pdqHash256FromFloatLuma(buffer1, buffer2, numRows, numCols, buffer64x64, buffer16x64, buffer16x16);
	}

	// ----------------------------------------------------------------
	// Made public for test/demo access
	public void fillFloatLumaFromBufferImage(BufferedImage img, float[] luma) // image numRows x numCols as row-major
																				// array
	{
		int numRows = img.getHeight();
		int numCols = img.getWidth();
	...
	// ----------------------------------------------------------------
	public HashAndQuality pdqHash256FromFloatLuma(float[] fullBuffer1, // image numRows x numCols as row-major array
			float[] fullBuffer2, // image numRows x numCols as row-major array
			int numRows, int numCols, float[][] buffer64x64, float[][] buffer16x64, float[][] buffer16x16) {
		...
		jaroszFilterFloat(fullBuffer1, fullBuffer2, numRows, numCols, windowSizeAlongRows, windowSizeAlongCols,
	...
	// ----------------------------------------------------------------
	public HashesAndQuality dihedralFromFile(String filename, HashingMetadata hashingMetadata, int dihFlags) // PDQ_DO_DIH_ORIGINAL
																												// et
																												// al.
			throws IOException {
		long t1, t2;
		...
		t1 = System.nanoTime();
		HashesAndQuality rv = dihedralFromBufferedImage(img, buffer1, buffer2, buffer64x64, buffer16x64, buffer16x16,
				buffer16x16Aux, dihFlags);
		t2 = System.nanoTime();
		hashingMetadata.hashSeconds = (float) ((t2 - t1) / 1e9);
	...
	// This is coded with an eye to the future wherein we could hash video frames,
	// all of which would be the same dimension, and the buffers could be reused.
	public HashesAndQuality dihedralFromBufferedImage(BufferedImage img, float[] buffer1, // image numRows x numCols as
																							// row-major array
			float[] buffer2, // image numRows x numCols as row-major array
			float[][] buffer64x64, float[][] buffer16x64, float[][] buffer16x16, float[][] buffer16x16Aux,
			int dihFlags) {
		int numRows = img.getHeight();
		int numCols = img.getWidth();
		...
		return pdqHash256esFromFloatLuma(buffer1, buffer2, numRows, numCols, buffer64x64, buffer16x64, buffer16x16,
				buffer16x16Aux, dihFlags);
	}

	// ----------------------------------------------------------------
	public HashesAndQuality pdqHash256esFromFloatLuma(float[] fullBuffer1, // image numRows x numCols as row-major array
			float[] fullBuffer2, // image numRows x numCols as row-major array
			int numRows, int numCols, float[][] buffer64x64, float[][] buffer16x64, float[][] buffer16x16,
			float[][] buffer16x16Aux, int dihFlags) {
		// Downsample (blur and decimate)
		int windowSizeAlongRows = computeJaroszFilterWindowSize(numCols);
		int windowSizeAlongCols = computeJaroszFilterWindowSize(numRows);
		...
		return new HashesAndQuality(hash, hashRotate90, hashRotate180, hashRotate270, hashFlipX, hashFlipY,
				hashFlipPlus1, hashFlipMinus1, quality);
	}

	// ----------------------------------------------------------------
	public static void decimateFloat(float[] in, // numRows x numCols in row-major order
			int inNumRows, int inNumCols, float[][] out) // 64x64
	{
	...
	void dct64To16(float[][] A, // input: 64x64
	...
	// ----------------------------------------------------------------
	void dct16OriginalToRotate90(float[][] A, // input 16x16
			float[][] B) // output 16x16
	{
		for (int i = 0; i < 16; i++) {
			for (int j = 0; j < 16; j++) {
	...
	void dct16OriginalToRotate180(float[][] A, // input 16x16
			float[][] B) // output 16x16
	{
		for (int i = 0; i < 16; i++) {
			for (int j = 0; j < 16; j++) {
	...
	void dct16OriginalToRotate270(float[][] A, // input 16x16
			float[][] B) // output 16x16
	{
		for (int i = 0; i < 16; i++) {
			for (int j = 0; j < 16; j++) {
	...
	void dct16OriginalToFlipX(float[][] A, // input 16x16
			float[][] B) // output 16x16
	{
		for (int i = 0; i < 16; i++) {
			for (int j = 0; j < 16; j++) {
	...
	void dct16OriginalToFlipY(float[][] A, // input 16x16
			float[][] B) // output 16x16
	{
		for (int i = 0; i < 16; i++) {
			for (int j = 0; j < 16; j++) {
	...
	void dct16OriginalToFlipPlus1(float[][] A, // input
			float[][] B) // output
	{
		for (int i = 0; i < 16; i++) {
			for (int j = 0; j < 16; j++) {
	...
	void dct16OriginalToFlipMinus1(float[][] A, // input 16x16
			float[][] B) // output 16x16
	{
		for (int i = 0; i < 16; i++) {
			for (int j = 0; j < 16; j++) {
	...
	Hash256 pdqBuffer16x16ToBits(float[][] dctOutput16x16) {
	...
	// ----------------------------------------------------------------
	public static void jaroszFilterFloat(float[] buffer1, // matrix as numRows x numCols in row-major order
			float[] buffer2, // matrix as numRows x numCols in row-major order
			int numRows, int numCols, int windowSizeAlongRows, int windowSizeAlongCols, int nreps) {
	...
	public static void box1DFloat(float[] invec, int inStartOffset, float[] outvec, int outStartOffset,
	...
	// ----------------------------------------------------------------
	public static void boxAlongRowsFloat(float[] in, // matrix as numRows x numCols in row-major order
			float[] out, // matrix as numRows x numCols in row-major order
			int numRows, int numCols, int windowSize) {
		for (int i = 0; i < numRows; i++) {
	...
	// ----------------------------------------------------------------
	public static void boxAlongColsFloat(float[] in, // matrix as numRows x numCols in row-major order
			float[] out, // matrix as numRows x numCols in row-major order
			int numRows, int numCols, int windowSize) {
		for (int j = 0; j < numCols; j++) {
